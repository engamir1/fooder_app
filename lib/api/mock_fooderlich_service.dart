...
class MockFooderlichService {
  ...
  Future<ExploreData> getExploreData() async {
    ...
    // print("the len is  ${friendPosts.length}");

    return ExploreData(friendPosts: friendPosts, todayRecipes: todayRecipes);
  }
  ...
  Future<List<ExploreRecipe>> _getTodayRecipes() async {
    ...
    final dataString = await _loadAsset('lib/data/sample_explore_recipes.json');
    // print("the data st is  $dataString");
    // Decode to json
  ...
  Future<List<Post>> _getFriendFeed() async {
    ...
    // Load json from file system
    final dataString = await _loadAsset('lib/data/sample_friends_feed.json');
    // Decode to json
    final Map<String, dynamic> json = jsonDecode(dataString);
  ...
  Future<List<SimpleRecipe>> getRecipes() async {
    ...
    await Future.delayed(const Duration(milliseconds: 100));
    // Load json from file system
    final dataString = await _loadAsset('lib/data/sample_recipes.json');
    // print(dataString);
    // Decode to json
    final Map<String, dynamic> json = jsonDecode(dataString);
